...
class UNETR8PS(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(self.vit.patch_embedding.patch_embeddings, torch.nn.Sequential):
            vit_weight_before = self.vit.blocks[0].mlp.linear1.weight.clone()
        else:
            vit_weight_before = self.vit.blocks[0].mlp.linear1.weight.clone()
        self.init_weights(revise_keys=revise_keys)
        vit_weight_after = self.vit.blocks[0].mlp.linear1.weight
        print("Are weights for linear1 before and after same: ", torch.equal(vit_weight_before, vit_weight_after))
